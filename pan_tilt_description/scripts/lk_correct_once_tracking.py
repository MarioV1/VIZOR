...
================================
ORB zoom-correction pipeline with Lucas-Kanade optical-flow tracking mode.

The zoom-correct pipeline (pressed with 'z') still uses ORB because wide-baseline
matching across zoom steps is what ORB is good at.  Once tracking starts (pressed
with 't'), the per-frame matching switches to pyramidal Lucas-Kanade optical flow
which is much faster and gives smoother trajectories on small frame-to-frame motion.
ORB is kept as the full-frame re-acquisition fallback after target loss.

Workflow:
  1. Draw ROI → 'c' save template
  2. 'z' → zoom + correct (ORB pipeline)
  3. 't' → start LK tracking at current zoom level
  4. 't' again or 'q' → stop tracking
  5. Everything else unchanged (WASD, r, k, 1/2/3, ESC, g)

Tracking design:
  - Runs in a background thread (does not block the display loop)
  - LK tracks ~100 corner features (Shi-Tomasi seed) frame-to-frame
  - Forward-backward error check rejects unreliable points each cycle
  - Re-seeds features when surviving count drops or on a periodic timer
  - Median displacement of surviving points → robust centroid measurement
  - Kalman filter (cx, cy, vx, vy) smooths and predicts during dropouts
  - Proportional controller with adaptive (zoom-aware) deadband
  - Lost-target re-acquisition uses ORB on an expanding search window
...
# ── Tracking parameters ────────────────────────────────────────────────────────
TRACK_HZ                  = 100    # control-loop frequency (Hz)
TRACK_DEADBAND_DEG        = 0.05  # angular deadband — no command below this error (degrees)
TRACK_KP                  = 1.0  # proportional gain — reduced to avoid overshoot
...
TRACK_KEYFRAME_S          = 1     # seconds between forced re-seeds (LK feature refresh)
TRACK_LOST_FRAMES         = 8     # consecutive failures before "target lost"
...
TRACK_MIN_INLIERS         = 12    # min inliers for ORB re-acquisition match
TRACK_MAX_JUMP_PX         = 80    # max px the target can move between cycles (distractor rejection)

# ── Lucas-Kanade optical flow parameters ───────────────────────────────────────
LK_MAX_POINTS             = 100   # target number of features to track
LK_MIN_POINTS             = 20    # re-seed when surviving points fall below this
LK_QUALITY_LEVEL          = 0.01  # Shi-Tomasi corner quality (0-1)
LK_MIN_DISTANCE           = 7     # min pixel distance between seed corners
LK_BLOCK_SIZE             = 7     # neighbourhood size for corner detection
LK_WIN_SIZE               = (21, 21)  # LK search window per pyramid level
LK_MAX_LEVEL              = 3     # pyramid levels (handles ~30-60 px displacements)
LK_FB_ERROR_PX            = 1.0   # max forward-backward round-trip error (px)
LK_TERM_CRITERIA          = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 30, 0.01)
...
class OrbCorrectOnce:
    ...
    def __init__(self):
        rospy.init_node('optflow_correct_once_tracking', anonymous=True)
        ...
        print("   Draw ROI → 'c' save template → 'z' zoom + correct → 't' LK track")
    ...
    def _tracking_loop(self):
        """
        Background thread: continuous Lucas-Kanade closed-loop tracking.

        Design decisions
        ----------------
        Event-driven frame consumption: the loop wakes on _new_frame_event
            instead of sleeping a fixed dt, so it never processes a stale
            frame twice and has zero unnecessary latency.

        LK feature tracking: Shi-Tomasi corners are seeded once at start
            (constrained by the Canny foreground mask) and then propagated
            frame-to-frame with cv2.calcOpticalFlowPyrLK.  A
            forward-backward error check rejects unreliable points each
            cycle.  When surviving points fall below LK_MIN_POINTS, or
            on a periodic timer, features are re-seeded from the current
            frame around the current target position.

        Robust centroid: the median displacement of surviving LK points
            is added to the previous target position.  Median is robust to
            outliers without needing RANSAC, and is sufficient for
            centroid-based servo control (no rotation/scale needed).

        Adaptive deadband: the deadband is expressed in degrees
            (TRACK_DEADBAND_DEG) and converted to pixels at the current
            zoom level so it represents the same angular tolerance
            regardless of zoom.

        Proportional control: error is scaled by TRACK_KP before
            converting to degrees, trading correction speed for
            mechanical stability.

        Lost-target & re-acquisition: after TRACK_LOST_FRAMES consecutive
            failures the thread falls back to ORB matching on a
            progressively expanding search window
            (TRACK_REACQUIRE_SIZES).  LK alone cannot recover from a
            true loss; ORB can.
        """
        ...
        # ── Foreground mask — built once at tracking start ─────────────────────
        ...
        if kf_mask is not None:
            ...
            print(f"  [Track] Canny mask active — features constrained to foreground")
        else:
            self._track_mask = None
            print(f"  [Track] Canny mask unavailable — using full epicenter window")

        # ── Build ORB re-acquisition template (used only after target loss) ────
        reacq_gray, reacq_lcx, reacq_lcy, _, _ = self._get_roi_crop_with_origin(
            prev_gray, current_cx, current_cy, epicenter, epicenter)
        reacq_template     = reacq_gray.copy()
        reacq_template_lcx = reacq_lcx
        reacq_template_lcy = reacq_lcy

        # ── Seed initial LK feature points ─────────────────────────────────────
        prev_pts = self._seed_lk_points(prev_gray, current_cx, current_cy,
                                        epicenter, kf_mask)
        if prev_pts is None or len(prev_pts) < LK_MIN_POINTS:
            print(f"❌ [Track] Could not seed enough LK points ({0 if prev_pts is None else len(prev_pts)})."
                  f" Aim at a more textured target.")
            self.tracking_active = False
            self.status = "Tracking failed: not enough features"
            return
        print(f"  [Track] Seeded {len(prev_pts)} LK feature points")

        last_seed_t      = time.time()
        fail_count       = 0
        reacquire_idx    = 0
        match_ms_cur     = 0.0   # match time this cycle (ms)
        match_hz_cur     = 0.0
        match_hz_min     = float('inf')
        ...
        while self.tracking_active and not rospy.is_shutdown():
            ...
            pred_cx = max(0, min(IMG_W - 1, float(predicted[0])))
            pred_cy = max(0, min(IMG_H - 1, float(predicted[1])))

            # ══════════════════════════════════════════════════════════════════
            # RE-ACQUISITION PATH (target was lost — use ORB on wide window)
            # ══════════════════════════════════════════════════════════════════
            ...
            if is_reacquiring:
                half = reacq_size
                sx1 = max(0, int(current_cx - half))
                sy1 = max(0, int(current_cy - half))
                sx2 = min(IMG_W, int(current_cx + half))
                sy2 = min(IMG_H, int(current_cy + half))
                scene_crop = gray_curr[sy1:sy2, sx1:sx2]

                t_match = time.time()
                result = find_template_point_masked(reacq_template, scene_crop,
                                                    reacq_template_lcx,
                                                    reacq_template_lcy,
                                                    kf_mask)
                ...
                if result is None:
                    fail_count += 1
                    if fail_count >= TRACK_LOST_FRAMES:
                        ...
                        if KALMAN_PREDICT_ON_LOSS:
                            ...
                            if err_pred > deadband_px:
                                ...
                                cap_x = TRACK_MAX_STEP_DEG * IMG_W / fov_h_now
                                cap_y = TRACK_MAX_STEP_DEG * IMG_H / fov_v_now
                                ex_c = max(-cap_x, min(cap_x, ex_pred * TRACK_KP))
                                ey_c = max(-cap_y, min(cap_y, ey_pred * TRACK_KP))
                ...
                # ORB re-acquired the target — reset to LK mode
                ...
                # Snap state to recovered position and re-seed LK
                kf.statePost[0] = cx_img
                kf.statePost[1] = cy_img
                kf.statePost[2] = 0.0
                kf.statePost[3] = 0.0
                current_cx, current_cy = float(cx_img), float(cy_img)
                prev_pts = self._seed_lk_points(gray_curr, current_cx, current_cy,
                                                epicenter, None)
                if prev_pts is None or len(prev_pts) < LK_MIN_POINTS:
                    prev_gray = gray_curr
                    continue
                ...
                with self._track_lock:
                    self._track_lost = False
                self.status = f"[Track] Re-acquired — {len(prev_pts)} new features"
                prev_gray = gray_curr
                # Fall through to control block below using cx_img, cy_img
                measurement_cx = float(cx_img)
                measurement_cy = float(cy_img)
                used_n         = int(n_inliers)
                used_label     = "ORB"
            ...
            # ══════════════════════════════════════════════════════════════════
                ...
                # Forward LK: prev_gray + prev_pts → curr_gray
                curr_pts, status_fwd, _ = cv2.calcOpticalFlowPyrLK(
                    prev_gray, gray_curr, prev_pts, None,
                    winSize=LK_WIN_SIZE, maxLevel=LK_MAX_LEVEL,
                    criteria=LK_TERM_CRITERIA)

                # Backward LK: curr_gray + curr_pts → prev_gray (FB check)
                back_pts, status_bwd, _ = cv2.calcOpticalFlowPyrLK(
                    gray_curr, prev_gray, curr_pts, None,
                    winSize=LK_WIN_SIZE, maxLevel=LK_MAX_LEVEL,
                    criteria=LK_TERM_CRITERIA)

                # Round-trip error per point
                fb_err = np.linalg.norm(prev_pts - back_pts, axis=2).reshape(-1)
                fwd_ok = status_fwd.reshape(-1).astype(bool)
                bwd_ok = status_bwd.reshape(-1).astype(bool)
                good   = fwd_ok & bwd_ok & (fb_err < LK_FB_ERROR_PX)
                ...
                if n_good < LK_MIN_POINTS:
                    # Too few survivors — count as a failure, try to re-seed
                    fail_count += 1
                    if fail_count >= TRACK_LOST_FRAMES:
                        reacquire_idx = 1   # jump straight to ORB wide-search
                        with self._track_lock:
                            self._track_lost = True
                    else:
                        # Try a quick re-seed at current position before giving up
                        new_pts = self._seed_lk_points(gray_curr,
                                                       current_cx, current_cy,
                                                       epicenter, None)
                        if new_pts is not None and len(new_pts) >= LK_MIN_POINTS:
                            prev_pts    = new_pts
                            last_seed_t = now
                            self.status = f"Tracking | re-seeded ({len(new_pts)} pts)"
                    current_cx, current_cy = pred_cx, pred_cy
                    prev_gray = gray_curr
                    continue

                # Median displacement of surviving points
                prev_good = prev_pts[good].reshape(-1, 2)
                curr_good = curr_pts[good].reshape(-1, 2)
                displacement = curr_good - prev_good
                dx_med = float(np.median(displacement[:, 0]))
                dy_med = float(np.median(displacement[:, 1]))

                measurement_cx = current_cx + dx_med
                measurement_cy = current_cy + dy_med

                # Distractor / runaway rejection
                jump_px = math.sqrt(dx_med**2 + dy_med**2)
                if jump_px > TRACK_MAX_JUMP_PX:
                ...
                # Update prev_pts to surviving curr_pts for next iteration
                prev_pts  = curr_good.reshape(-1, 1, 2).astype(np.float32)
                used_n    = n_good
                used_label = "LK"
                fail_count = 0
                with self._track_lock:
                ...
                # Periodic re-seed to fight feature drift
                if (now - last_seed_t) >= TRACK_KEYFRAME_S or len(prev_pts) < LK_MAX_POINTS // 2:
                    new_pts = self._seed_lk_points(gray_curr,
                                                   measurement_cx, measurement_cy,
                                                   epicenter, None)
                    if new_pts is not None and len(new_pts) >= LK_MIN_POINTS:
                        prev_pts    = new_pts
                        last_seed_t = now
            ...
            # ══════════════════════════════════════════════════════════════════
            # COMMON: Kalman update + control
            # ══════════════════════════════════════════════════════════════════
            measurement = np.array([[measurement_cx], [measurement_cy]],
                                   dtype=np.float32)
            kf.correct(measurement)

            # Clamp Kalman velocity to prevent runaway
            ...
            if err_px > deadband_px:
                ...
                # After a motor command, re-seed LK on the next iteration so
                # features come from the post-motion frame, not the pre-motion one
                last_seed_t = 0.0
            ...
            self.status = (f"Tracking | err={err_px:.1f}px ({err_deg:.3f} deg)"
                           f" | {used_label}={used_n}"
                           f" | {match_ms_cur:.1f}ms {match_hz_cur:.0f}Hz"
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # LK feature seeding helper
    # ══════════════════════════════════════════════════════════════════════════
    def _seed_lk_points(self, gray, cx, cy, epicenter, fg_mask):
        """Detect Shi-Tomasi corners inside an epicenter window around (cx,cy).

        If fg_mask is provided it must be the same size as the epicenter crop
        and is used to constrain detection to foreground pixels.
        Returns an Nx1x2 float32 array of points in *full-image* coordinates,
        or None if too few corners were found.
        """
        x1 = max(0, int(cx - epicenter / 2.0))
        y1 = max(0, int(cy - epicenter / 2.0))
        x2 = min(IMG_W, int(cx + epicenter / 2.0))
        ...
        mask_arg = None
        if fg_mask is not None and fg_mask.shape[:2] == crop.shape[:2]:
            mask_arg = fg_mask

        pts = cv2.goodFeaturesToTrack(
            crop,
            maxCorners=LK_MAX_POINTS,
        ...
        # Remap from crop coords to full-image coords
        pts[:, 0, 0] += x1
        pts[:, 0, 1] += y1
