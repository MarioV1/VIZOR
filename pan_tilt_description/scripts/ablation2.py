...
=========================
PTZ zoom correction ablation study — Optical Flow vs Open-Loop Kinematics.
Runs sequential testing (40 iterations total: 20 Complete + 20 Suppressed),
...
METHOD_NAMES = ["OptFlow_Completo", "Sin_OptFlow"]
...
class OpticalFlowBenchmark:
    ...
    def __init__(self):
        ...
        print("\n🟢 Ablation Study Iniciado (OptFlow Completo vs Cinemática Pura)")
    ...
    def _run_optflow(self, target_zoom, cx_cursor, cy_cursor, x, y, orig_w, orig_h):
        ...
        # === DUAL TRACKING UNIVERSE SETUP ===
        # real_cx: Ground Truth trackeado por OptFlow
        # math_cx: Proyección puramente cinemática (Bucle Abierto)
        real_cx, real_cy = initial_cx, initial_cy
        math_cx, math_cy = initial_cx, initial_cy

        self._update_roi_visual(real_cx, real_cy, 1.0, orig_w, orig_h)
        ...
        for step_zoom in zoom_steps:
            ...
            # 1. Proyección Matemática (Lo que asume el Bucle Abierto)
            fov_h1, _ = self._get_fovs(zoom_prev)
            fov_h2, _ = self._get_fovs(step_zoom)
            scale = fov_h1 / fov_h2
            M = cv2.getRotationMatrix2D((IMG_W / 2.0, IMG_H / 2.0), 0, scale)
            pt_math = np.float32([[[math_cx, math_cy]]])
            math_cx, math_cy = cv2.transform(pt_math, M)[0][0]

            # 2. Sensor Ground Truth (Optical Flow)
            roi_x1, roi_y1, roi_w, roi_h = self._get_roi_bbox(real_cx, real_cy, EPICENTER_SIZE, EPICENTER_SIZE)
            result = self._find_point_optical_flow(
                prev_gray, scene_gray_before, real_cx, real_cy,
                roi_x1, roi_y1, roi_w, roi_h, zoom_prev, step_zoom
            )
            match_duration = time.time() - t_start

            if result is None:
                self.status = f"❌ [{self.current_method}] OptFlow Ground Truth perdido en x{step_zoom:.1f}"
                return results

            real_cx, real_cy, n_inliers, H_final, _, _, _, _ = result

            # 3. Ablation Switch: ¿Quién controla la cámara?
            if self.current_method == "Sin_OptFlow":
                active_cx, active_cy = math_cx, math_cy
            else:
                active_cx, active_cy = real_cx, real_cy
                math_cx, math_cy = real_cx, real_cy # Evita divergencia en modo completo
                
            ex_cmd = active_cx - cx_cursor
            ey_cmd = active_cy - cy_cursor
            
            # El error real para el Benchmark SIEMPRE se mide desde Ground Truth
            ex_real = real_cx - cx_cursor
            ey_real = real_cy - cy_cursor
            
            err_px = math.sqrt(ex_real**2 + ey_real**2)
            deg_x, deg_y = self._px_to_deg(ex_real, ey_real, step_zoom)
            err_deg = math.sqrt(deg_x**2 + deg_y**2)

            results.append({
                "zoom": step_zoom, "label": f"x{step_zoom:.1f}",
                "ex": ex_real, "ey": ey_real, "err_px": err_px,
                "ex_deg": deg_x, "ey_deg": deg_y, "err_deg": err_deg,
                "duration_s": match_duration
            })

            # Micro-corrección mecánica si estamos desalineados
            if abs(ex_cmd) > MICRO_CORRECT_PX or abs(ey_cmd) > MICRO_CORRECT_PX:
                self._calculate_and_send_direct_cmd(ex_cmd, ey_cmd, step_zoom)
                time.sleep(2.0)

                frame_z_after = self._get_frame()
                scene_gray_after = cv2.cvtColor(frame_z_after, cv2.COLOR_BGR2GRAY)
                
                # El modelo matemático asume que los motores ejecutaron el comando perfectamente
                math_cx -= ex_cmd
                math_cy -= ey_cmd

                # El sensor Ground Truth mide la realidad del salto mecánico
                roi_x1_mc, roi_y1_mc, roi_w_mc, roi_h_mc = self._get_roi_bbox(real_cx, real_cy, EPICENTER_SIZE, EPICENTER_SIZE)
                res_centered = self._find_point_optical_flow(
                    scene_gray_before, scene_gray_after, real_cx, real_cy,
                    roi_x1_mc, roi_y1_mc, roi_w_mc, roi_h_mc, step_zoom, step_zoom,
                    guess_dx=-ex_cmd, guess_dy=-ey_cmd
                )

                if res_centered is not None:
                    real_cx, real_cy = res_centered[0], res_centered[1]
                else:
                    real_cx -= ex_cmd
                    real_cy -= ey_cmd # Fallback
            ...
            if self.current_method == "Sin_OptFlow":
                ...
                active_cx, active_cy = real_cx, real_cy
                math_cx, math_cy = real_cx, real_cy

            self._update_roi_visual(active_cx, active_cy, step_zoom, orig_w, orig_h)

            if step_zoom != zoom_steps[-1]:
                prev_gray = scene_gray_final
                zoom_prev = step_zoom

        self.last_target_px = (active_cx, active_cy)

        # ── Comando PTZ Final ──
        ex_final, ey_final = active_cx - cx_cursor, active_cy - cy_cursor
        yaw_final, pitch_final = self._calculate_and_send_direct_cmd(ex_final, ey_final, target_zoom)

        # ── Medición Final Post-Corrección (Ground Truth Realidad) ──
        time.sleep(2.0)
        frame_post = self._get_frame()
        if frame_post is not None:
            scene_post = cv2.cvtColor(frame_post, cv2.COLOR_BGR2GRAY)
            roi_x1_p, roi_y1_p, roi_w_p, roi_h_p = self._get_roi_bbox(real_cx, real_cy, EPICENTER_SIZE, EPICENTER_SIZE)
            res_post = self._find_point_optical_flow(
                scene_gray_final, scene_post, real_cx, real_cy,
            ...
            if res_post is not None:
                real_cx_post, real_cy_post = res_post[0], res_post[1]
                ex_post = real_cx_post - cx_cursor
                ey_post = real_cy_post - cy_cursor
                ...
                self.last_target_px = (real_cx_post, real_cy_post)
    ...
    def _save_benchmark_results(self, roi, init_yaw, init_pitch, out_dir, ts):
        ...
        # Color azul para OptFlow Completo, Naranja para Sin OptFlow
        ...
        method_colors = {'OptFlow_Completo': '#1f77b4', 'Sin_OptFlow': '#ff7f0e'}
        method_markers = {'OptFlow_Completo': 'o', 'Sin_OptFlow': 'X'}
