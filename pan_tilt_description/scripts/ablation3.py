...
# -*- coding: utf-8 -*-
"""
optical_flow_benchmark_steps.py
===============================
PTZ zoom correction ablation study — Incremental Steps vs Direct Jump.
Runs sequential testing (40 iterations total: 20 Incremental + 20 Direct),
generating success rate charts, penalized boxplots, and specialized scatter plots.
"""
...
PRE_CENTRE_PX = 40
N_REPEATS = 20  # 20 iteraciones por método
PENALTY_PX = 600.0 # Error máximo asignado cuando el tracker se pierde

# ABLATION STUDY METHODS
METHOD_NAMES = ["OptFlow_Incremental", "OptFlow_Directo"]
...
class OpticalFlowBenchmark:
    ...
    def __init__(self):
        ...
        print("\n🟢 Ablation Study Iniciado (Incremental Steps vs Direct Jump)")
        print(f"   Se realizarán {N_REPEATS} repeticiones por cada modelo (Total: {N_REPEATS*2} zooms).")
    ...
    def _create_penalty_result(self, target_zoom, step_zoom, label):
        """Genera un diccionario de error máximo para cuando el tracker falla."""
        deg_x, deg_y = self._px_to_deg(PENALTY_PX, PENALTY_PX, target_zoom)
        penalty_deg = math.sqrt(deg_x**2 + deg_y**2)
        return {
            "zoom": step_zoom, "label": label,
            "ex": PENALTY_PX, "ey": PENALTY_PX, "err_px": PENALTY_PX,
            "ex_deg": deg_x, "ey_deg": deg_y, "err_deg": penalty_deg,
            "duration_s": 0.0, "is_post_correction": True, "success": False
        }
    ...
    def _run_optflow(self, target_zoom, cx_cursor, cy_cursor, x, y, orig_w, orig_h):
        ...
        if abs(ex_init) > PRE_CENTRE_PX or abs(ey_init) > PRE_CENTRE_PX:
            ...
            if res0 is None:
                ...
                results.append(self._create_penalty_result(target_zoom, 1.0, "PreCenter_FAIL"))
                return results
            
            initial_cx, initial_cy = res0[0], res0[1]
            frame_base = frame1
        else:
            frame_base = frame_orig

        self._update_roi_visual(initial_cx, initial_cy, 1.0, orig_w, orig_h)
        prev_gray = cv2.cvtColor(frame_base, cv2.COLOR_BGR2GRAY)
        zoom_prev = 1.0

        # --- ABLATION LOGIC: Incremental vs Directo ---
        if self.current_method == "OptFlow_Directo":
            zoom_steps = [float(round(target_zoom, 1))]
        else:
            MAX_SAFE_JUMP = 2.0
        ...
        for step_zoom in zoom_steps:
            ...
            if result is None:
                self.status = f"❌ [{self.current_method}] Tracker Perdido en x{step_zoom:.1f}"
                results.append(self._create_penalty_result(target_zoom, step_zoom, f"x{step_zoom:.1f}_FAIL"))
                return results

            cx_found, cy_found, _, _, _, _, _, _ = result
            ...
            results.append({
                ...
                "duration_s": match_duration, "success": True
            })

            # Micro-corrección incremental solo en modo progresivo
            if (abs(ex_step) > MICRO_CORRECT_PX or abs(ey_step) > MICRO_CORRECT_PX) and self.current_method != "OptFlow_Directo":
                ...
                if res_centered is not None:
                    cx_found, cy_found = res_centered[0], res_centered[1]
                else:
        ...
        # ── Medición Final Post-Corrección ──
        time.sleep(2.0)
        ...
        if frame_post is not None:
            ...
            if res_post is not None:
                ...
                results.append({
                    ...
                    "duration_s": 0.0, "is_post_correction": True, "success": True
                })
                self.last_target_px = (cx_post, cy_post)
                self.last_error_px = (ex_post, ey_post)
            else:
                self.status = f"❌ [{self.current_method}] Fallo Post-Corrección"
                results.append(self._create_penalty_result(target_zoom, target_zoom, "PostCorrect_FAIL"))
                ...
                for rep in range(1, N_REPEATS + 1):
                    ...
                    if results:
                        ...
                        final_errors[method].append({
                            ...
                            "cx_actual": cx_cursor + final["ex"] if final["ex"] is not None else None,
                            "cy_actual": cy_cursor + final["ey"] if final["ey"] is not None else None,
                        })
                        
                        if final.get("success", False):
                            print(f"    ✅ Éxito | err: {final['err_deg']:.4f}° ({final['err_px']:.1f}px)")
                        else:
                            print(f"    ❌ Fallo | Penalización Máxima Aplicada")
    ...
    def _save_benchmark_results(self, roi, init_yaw, init_pitch, out_dir, ts):
        ...
        with open(csv_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["# Ablation Study: Incremental vs Direct", ts, f"N_REPEATS={N_REPEATS}"])
            w.writerow(["method","repeat","zoom","success","label","ex_px","ey_px","err_px","ex_deg","ey_deg","err_deg","duration_s"])
            for mname, records in self.benchmark_results.items():
                if not records: continue
                for r in records:
                    w.writerow([mname, r.get("repeat",""), r["zoom"], r.get("success", False), r["label"],
        ...
        success_rates = []
        colors = ['#1f77b4', '#d62728'] # Azul para Incremental, Rojo para Directo
        ...
        for mname in METHOD_NAMES:
            ...
            success_rates.append((n_success / max(1, len(records))) * 100)
            
            # Todos los errores entran al boxplot, incluyendo los de 600px (penalización)
            data_err_px.append([r["err_px"] for r in records])
            data_ex_px.append([r["ex_px"]   for r in records])
            data_ey_px.append([r["ey_px"]   for r in records])

        title = f"Ablation Study (Steps vs Direct)\n{N_REPEATS} repeats/method | zoom x{self.current_target_zoom:.1f}"

        # ── GRÁFICA 1: TASA DE ÉXITO (ROBUSTEZ) ──
        fig1, ax1 = plt.subplots(figsize=(6, 5))
        bars = ax1.bar(labels, success_rates, color=colors, edgecolor='black', alpha=0.8)
        ax1.set_ylim(0, 110)
        ax1.set_ylabel("Porcentaje de Éxito (%)")
        ax1.set_title(f"Robustez del Tracking\n¿Sobrevivió al salto de zoom?", fontsize=11)
        ax1.grid(axis='y', linestyle='--', alpha=0.7)
        for bar in bars:
            height = bar.get_height()
            ax1.text(bar.get_x() + bar.get_width()/2., height + 2, f'{height:.1f}%', ha='center', va='bottom', fontweight='bold')
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, "1_tasa_de_exito.png"), dpi=150)
        plt.close()

        # ── GRÁFICA 2: BOXPLOTS CON PENALIZACIÓN ──
        fig, axes = plt.subplots(1, 3, figsize=(16, 7))
        fig.suptitle(title + "\nDistribución de Error (Incluyendo Penalización por Pérdida)", fontsize=11, y=1.02)
        ...
        for ax, data, ttl, ylabel in [
            ...
            # Línea de la zona de penalización
            ax.axhline(PENALTY_PX, color="red", linestyle="--", alpha=0.6, label="Zona de Pérdida (Penalty)")
            if ttl == "Euclidean error": ax.legend(loc="lower right")
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, "2_boxplots_penalizados.png"), dpi=150, bbox_inches="tight"); plt.close()

        # ── GRÁFICA 3: SCATTER COMPARATIVO DIANA ──
        fig3, ax3 = plt.subplots(figsize=(8, 8))
        ax3.set_aspect('equal')
        ax3.set_title(title + "\nPrecisión de impactos (Diana)", fontsize=11)

        max_radius = PENALTY_PX * 1.15
        
        # Anillos de precisión concéntricos
        for i in range(1, 4):
            ax3.add_patch(plt.Circle((0, 0), 50 * i, fill=False, color='green', linestyle=':', alpha=0.3))
        # Anillo de pérdida (Penalty Ring)
        ax3.add_patch(plt.Circle((0, 0), PENALTY_PX, fill=False, color='red', linestyle='-', linewidth=2, alpha=0.5))
        ax3.text(10, PENALTY_PX + 15, "ZONA DE PÉRDIDA (Target Lost)", color='red', fontweight='bold')

        ax3.axhline(0, color='gray', linewidth=0.5, alpha=0.5)
        ax3.axvline(0, color='gray', linewidth=0.5, alpha=0.5)
        ax3.plot(0, 0, 'o', color='green', markersize=12, zorder=10, label='Target Ideal')

        for mname, color in zip(METHOD_NAMES, colors):
            records = self.final_errors.get(mname, [])
            valid_recs = [r for r in records if r["success"]]
            failed_recs = [r for r in records if not r["success"]]
            
            # Graficar aciertos (Puntos normales cerca del centro)
            if valid_recs:
                xs = [r["ex_px"] for r in valid_recs]
                ys = [r["ey_px"] for r in valid_recs]
                ax3.scatter(xs, ys, c=color, marker='o', s=90, linewidths=1.5, zorder=5, label=f"{mname} (Éxito)", alpha=0.8)

            # Graficar fallos (Repartidos en forma de anillo en la línea de penalización)
            if failed_recs:
                fail_xs, fail_ys = [], []
                n_fails = len(failed_recs)
                for idx in range(n_fails):
                    # Distribuir equitativamente en 360 grados a una distancia PENALTY_PX
                    angle = idx * (2 * math.pi / max(1, n_fails)) + (math.pi/4 if mname == "OptFlow_Directo" else 0)
                    fail_xs.append(PENALTY_PX * math.cos(angle))
                    fail_ys.append(PENALTY_PX * math.sin(angle))
                # Graficados con una "X" gigante
                ax3.scatter(fail_xs, fail_ys, c=color, marker='X', s=180, linewidths=1.5, zorder=6, label=f"{mname} (Perdido)", alpha=0.9)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, "3_diana_impactos.png"), dpi=150, bbox_inches="tight"); plt.close()
