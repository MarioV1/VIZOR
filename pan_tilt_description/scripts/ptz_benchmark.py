...
ptz_benchmark.py
================
Combined PTZ zoom correction benchmark — ORB + Optical Flow (Lucas-Kanade).
Sequential testing with camera pose save/restore between methods,
error boxplots in degrees, execution time comparison, and continuous
...
class PTZBenchmark:
    ...
    def __init__(self):
        rospy.init_node('ptz_benchmark', anonymous=True)
        ...
        cv2.namedWindow("PTZ Benchmark")
        cv2.setMouseCallback("PTZ Benchmark", self._mouse_cb)

        print("\n🟢 ptz_benchmark started (ORB + OpticalFlow)")
    ...
    def _calculate_and_send_tf_cmd(self, ex, ey, zoom_level):
        ...
        try:
            transform = self.tf_buffer.lookup_transform(self.TF_BASE, self.TF_CAMERA, rospy.Time(0), rospy.Duration(1.0))
            target_base = tf2_geometry_msgs.do_transform_point(target_cam, transform)

            cam_x = transform.transform.translation.x
            cam_y = transform.transform.translation.y
            cam_z = transform.transform.translation.z

            dx = target_base.point.x - cam_x
            dy = target_base.point.y - cam_y
            dz = target_base.point.z - cam_z

            pan_deg = math.degrees(math.atan2(dy, dx))
            tilt_deg = math.degrees(math.atan2(dz, math.sqrt(dx**2 + dy**2)))

            new_yaw = max(self.PAN_MIN_DEG, min(self.PAN_MAX_DEG, pan_deg))
            new_pitch = max(self.TILT_MIN_DEG, min(self.TILT_MAX_DEG, -tilt_deg))
    ...
    # ── ORB correction pipeline ───────────────────────────────────────────────

    def _run_orb(self, target_zoom, cx_cursor, cy_cursor, x, y, orig_w, orig_h):
        """Run ORB correction pipeline. Returns list of result dicts."""
        results = []
        ...
        if abs(ex_init) > PRE_CENTRE_PX or abs(ey_init) > PRE_CENTRE_PX:
            ...
            res0 = find_template_point(template_orig, scene_gray_1, local_cx_orig, local_cy_orig)
            if res0 is None:
        ...
        for step_zoom in zoom_steps:
            ...
            if abs(ex_step) > MICRO_CORRECT_PX or abs(ey_step) > MICRO_CORRECT_PX:
                ...
                res_centered = find_template_point(template_gray, scene_gray, local_cx, local_cy)
        ...
        if frame_post is not None:
            ...
            if res_post is not None:
                ...
                # Replace the last result with post-correction measurement
                results.append({
    ...
    def _run_optflow(self, target_zoom, cx_cursor, cy_cursor, x, y, orig_w, orig_h):
        ...
        if frame_post is not None:
            scene_post = cv2.cvtColor(frame_post, cv2.COLOR_BGR2GRAY)
            # Use the last scene before correction as reference
            last_scene = scene_gray_final if 'scene_gray_final' in dir() else (scene_gray_before if 'scene_gray_before' in dir() else None)
            if last_scene is not None:
                ...
                res_post = self._find_point_optical_flow(
                    last_scene, scene_post, current_cx, current_cy,
    ...
    def _zoom_and_correct(self):
        try:
            ...
            # Collect final-zoom errors across all repeats for each method
            # Each entry: {repeat, ex_px, ey_px, err_px, ex_deg, ey_deg, err_deg, duration_s}
            final_errors = {m: [] for m in METHOD_NAMES}
            # Also keep all per-step results for the full CSV
            all_step_results = {m: [] for m in METHOD_NAMES}

            # ── Run each method N_REPEATS times ──
            for method in METHOD_NAMES:
                ...
                for rep in range(1, N_REPEATS + 1):
                    ...
                    # Restore pose and ROI before each repeat
                    self.roi = orig_roi
            ...
            self.current_method = ""
            self.status = f"✅ Benchmark complete — {N_REPEATS}x per method"
            print(f"\n{self.status}\n")
    ...
    def _save_benchmark_results(self, roi, init_yaw, init_pitch, out_dir, ts):
        ...
        method_colors = {'ORB': '#2196F3', 'OpticalFlow': '#4CAF50'}
        method_markers = {'ORB': 'P', 'OpticalFlow': 'X'}
